...
class ComponentSlider extends Component {
	render() {
		const { data, renderComponent, style } = this.props;
...
class ComponentSlider2 extends Component {
	state = {
		position: 0,
		...
		return (
			<View style={[styles.container, style]} onLayout={this._onLayout}>
				<ScrollView
					...
					contentInset={
						loopBothSides ? { left: this.state.width } : {}
					}
					horizontal={true}
				...
				>
					{loopBothSides &&
						this._renderImage(images[images.length - 1], -1)}
